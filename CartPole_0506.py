...
class DQNAgent:
    ...
    def train(self):
        ...
        current_q =self.Q_eval(states).gather(1,actions)
...
if __name__ == "__main__":
    ...
    agent=DQNAgent(state_dim, action_dim, lr=1e-3, gamma=0.99, epsilon=1.0, batch_size=128)
    swanlab.init(
        project="CartPole-v1",
    ...
    for episode in tqdm(range(swanlab.config["episode"])):
        ...
        while True:
            ...
            next_state, reward, terminated, truncated ,_ = env.step(action)
            # increment_reward=1-step/MAX_STEP
            # reward=reward*increment_reward
            if step_train %10 ==0:    
                reward+=10
            if step_train %100==0:
                reward+=100
        ...
        if episode % 10 == 0:
            ...
            if avg_reward >= agent.best_avg_reward:
                ...
                agent.save_model("0506_DQN_CartPole/best_model.pth")
